...
void ipc_handle_frame(uint8_t type, uint8_t ep_addr, uint16_t seq,
    ...
    case FRAME_DEVICE_READY: {
        // Locate the real keyboard's IN endpoint in the relayed config
        // descriptor so km_apply knows which endpoint to overlay.
        uint8_t  kbd_ep;
        uint16_t kbd_mps;
        if (desc_config_valid &&
            cfgdesc_find_kbd_in_ep(desc_config, desc_config_len, &kbd_ep, &kbd_mps)) {
            km_set_kbd_endpoint(kbd_ep, kbd_mps);
        }
        device_ready = true;
        poke_main = true;
        break;
